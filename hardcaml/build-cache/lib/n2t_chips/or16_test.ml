...
module Circuit = User_circuit.Or16
...
let%expect_test "Or16 gate" =
  ...
  let test a b expected =
    ...
    if out = expected then begin
      incr passed;
      printf "PASS: Or16(0x%04X, 0x%04X) = 0x%04X\n" a b out
    end else begin
      incr failed;
      printf "FAIL: Or16(0x%04X, 0x%04X) = 0x%04X, expected 0x%04X\n" a b out expected
    end
  in

  test 0x0000 0x0000 0x0000;
  test 0xFFFF 0xFFFF 0xFFFF;
  test 0x0000 0xFFFF 0xFFFF;
  test 0xFFFF 0x0000 0xFFFF;
