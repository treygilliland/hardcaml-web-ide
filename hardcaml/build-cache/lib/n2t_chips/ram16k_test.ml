...
module Circuit = User_circuit.Ram16k
...
let%expect_test "RAM16K" =
  ...
  let write addr value =
    ...
    inputs.address := Bits.of_int_trunc ~width:14 addr;
  ...
  let read addr expected =
    ...
    if out = expected then begin
      incr passed;
      printf "PASS: RAM16K[%d] = 0x%04X\n" addr out
    end else begin
      incr failed;
      printf "FAIL: RAM16K[%d] = 0x%04X, expected 0x%04X\n" addr out expected
    end
  in
