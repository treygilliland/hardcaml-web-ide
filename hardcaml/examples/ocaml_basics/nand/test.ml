...
let test_nand outputs expected =
...
let run_testbench (sim : Harness.Sim.t) =
  ...
  (* Test 1: 0 NAND 0 = 1 (NOT (0 AND 0) = NOT 0 = 1) *)
  inputs.a := Bits.gnd;
  inputs.b := Bits.gnd;
  cycle ();
  test_nand outputs 1;

  (* Test 2: 0 NAND 1 = 1 (NOT (0 AND 1) = NOT 0 = 1) *)
  inputs.a := Bits.gnd;
  inputs.b := Bits.vdd;
  cycle ();
  test_nand outputs 1;

  (* Test 3: 1 NAND 0 = 1 (NOT (1 AND 0) = NOT 0 = 1) *)
  inputs.a := Bits.vdd;
  inputs.b := Bits.gnd;
  cycle ();
  test_nand outputs 1;

  (* Test 4: 1 NAND 1 = 0 (NOT (1 AND 1) = NOT 1 = 0) *)
  inputs.a := Bits.vdd;
  inputs.b := Bits.vdd;
  cycle ();
  test_nand outputs 0;
...
let%expect_test "Hello Nand Gate test" =
  passed := 0;
  failed := 0;
