...
let test_output name expected actual =
...
let run_testbench (sim : Harness.Sim.t) =
  ...
  let equal_val = Bits.to_int_trunc !(outputs.Circuit.O.equal) in
  
  test_output "and" (5 land 3) and_val;  (* 5 & 3 = 1 *)
  test_output "or" (5 lor 3) or_val;    (* 5 | 3 = 7 *)
  test_output "xor" (5 lxor 3) xor_val;   (* 5 ^ 3 = 6 *)
  test_output "not_a" ((lnot 5) land 0xF) not_a_val;  (* ~5 & 0xF = 10 *)
  test_output "sum" (5 + 3) sum_val;      (* 5 + 3 = 8 *)
  test_output "equal" 0 equal_val;          (* 5 != 3, so equal = 0 *)
  ...
  let equal_val2 = Bits.to_int_trunc !(outputs.Circuit.O.equal) in
  test_output "equal" 1 equal_val2;  (* 7 == 7, so equal = 1 *)
