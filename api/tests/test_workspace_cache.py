...
def test_create_new_workspace(cache):
    ...
    path, is_hit = workspace_cache.get_or_create(
        "session-1", is_n2t=False, template_dir=template
    )
...
def test_reuse_existing_workspace(cache):
    ...
    # First request
    path1, is_hit1 = workspace_cache.get_or_create(
        "session-1", is_n2t=False, template_dir=template
    )
    assert not is_hit1

    # Second request with same session
    path2, is_hit2 = workspace_cache.get_or_create(
        "session-1", is_n2t=False, template_dir=template
    )
    assert is_hit2
    assert path1 == path2
...
def test_lru_eviction(cache):
    ...
    path1, _ = workspace_cache.get_or_create(
        "session-1", is_n2t=False, template_dir=template
    )
    path2, _ = workspace_cache.get_or_create(
        "session-2", is_n2t=False, template_dir=template
    )
    path3, _ = workspace_cache.get_or_create(
        "session-3", is_n2t=False, template_dir=template
    )
    ...
    path4, is_hit = workspace_cache.get_or_create(
        "session-4", is_n2t=False, template_dir=template
    )
...
def test_update_workspace_files(cache):
    ...
    path, _ = workspace_cache.get_or_create(
        "session-1", is_n2t=False, template_dir=template
    )
...
def test_project_type_change_recreates_workspace(cache):
    ...
    path2, is_hit2 = workspace_cache.get_or_create(
        "session-1", is_n2t=True, template_dir=template
    )
